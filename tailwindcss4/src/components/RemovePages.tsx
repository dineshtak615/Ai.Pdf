...
import { FaFilePdf, FaSpinner, FaTimes, FaDownload, FaCloudUploadAlt, FaInfoCircle, FaListUl } from 'react-icons/fa';
...
const RemovePages = () => {
    ...
    const handleRemovePages = async (e?: FormEvent) => {
        if (e) {
            e.preventDefault();
        }
        
        if (!file || !pages.trim()) {
            setStatus({ loading: false, error: "Please select a file and enter pages to remove" });
            return;
        }
        ...
        try {
            ...
            const res = await axios.post(
                `${import.meta.env.VITE_API_URL}/remove-pages`,
                formData,
                {
            ...
            setStatus({ loading: false, error: "Failed to remove pages. Please try again." });
        }
    };

    return (
        <div className="max-w-4xl mx-auto mt-8 p-8 bg-white dark:bg-gray-900 rounded-2xl shadow-xl">
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-6 text-center">Remove PDF Pages</h1>

            <div className="text-gray-600 dark:text-gray-300 mb-8 text-center">
                Remove specific pages from your PDF document.
            </div>

            <form onSubmit={handleRemovePages} className="space-y-6">
                {/* File Upload */}
                <div>
                ...
                <div>
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                        Use commas to separate page numbers and hyphens for ranges. Example: "2,4-6"
                    </p>
                </div>
            ...
            <div className="mt-12 p-6 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-2xl border border-blue-100 dark:border-blue-800/30">
                ...
                <ol className="list-decimal list-inside space-y-2 text-blue-700 dark:text-blue-300">
                    ...
                    <li>Enter the page numbers you want to remove in the "Pages to Remove" field (e.g., "2,4-6").</li>
                    <li>Click the "Remove Pages" button.</li>
                    <li>Once the process is complete, a download link will appear. Click it to download your modified PDF file.</li>
                </ol>
            </div>
        </div>
    );
