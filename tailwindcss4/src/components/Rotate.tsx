...
import { FaFilePdf, FaSpinner, FaTimes, FaDownload, FaCloudUploadAlt, FaInfoCircle, FaRedo, FaListOl } from 'react-icons/fa';
...
const Rotate = () => {
    ...
    const handleRotate = async () => {
        if (!file || !pages.trim() || !angle) {
            setStatus({ loading: false, error: "Please select a file, enter pages, and select angle" });
            return;
        }
        ...
        try {
            ...
            const res = await axios.post(
                `${import.meta.env.VITE_API_URL}/rotate`,
                formData,
                {
            ...
            setStatus({ loading: false, error: "Failed to rotate pages. Please try again." });
        }
    };

    return (
        <div className="max-w-4xl mx-auto mt-8 p-8 bg-white dark:bg-gray-900 rounded-2xl shadow-xl">
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-6 text-center">Rotate PDF Pages</h1>

            <div className="text-gray-600 dark:text-gray-300 mb-8 text-center">
                Rotate specific pages or the entire PDF document.
            </div>

            <form onSubmit={(e) => { e.preventDefault(); handleRotate(); }} className="space-y-6">
                {/* File Upload */}
                <div>
                ...
                <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                        ...
                        Pages to Rotate (e.g., "1,3-5" or leave empty for all pages)
                    </label>
                    <input
                        type="text"
                        value={pages}
                        onChange={(e) => setPages(e.target.value)}
                        placeholder="Enter page numbers/ranges"
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                        Use commas to separate page numbers and hyphens for ranges. Example: "1,3-5"
                    </p>
                </div>
                ...
                <button
                    type="submit"
                    disabled={status.loading || !file || !angle}
            ...
            <div className="mt-12 p-6 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-2xl border border-blue-100 dark:border-blue-800/30">
                ...
                <ol className="list-decimal list-inside space-y-2 text-blue-700 dark:text-blue-300">
                    ...
                    <li>Enter the page numbers you want to rotate in the "Pages to Rotate" field (e.g., "1,3-5").</li>
                    <li>Select the desired rotation angle.</li>
                    <li>Click the "Rotate Pages" button.</li>
                    <li>Once the process is complete, a download link will appear. Click it to download your rotated PDF file.</li>
                </ol>
            </div>
        </div>
    );
