...
import { FaFilePdf, FaSpinner, FaTimes, FaDownload, FaCloudUploadAlt, FaInfoCircle, FaListOl } from 'react-icons/fa';
...
const ExtractPages = () => {
    ...
    const handleExtractPages = async (e?: FormEvent) => {
        if (e) {
            e.preventDefault();
        }
        
        if (!file || !pages.trim()) {
            setStatus({ loading: false, error: "Please select a file and enter pages to extract" });
            return;
        }
        ...
        try {
            ...
            const res = await axios.post(
                `${import.meta.env.VITE_API_URL}/extract-pages`,
                formData,
                {
            ...
            setStatus({ loading: false, error: "Failed to extract pages. Please try again." });
        }
    };

    return (
        <div className="max-w-4xl mx-auto mt-8 p-8 bg-white dark:bg-gray-900 rounded-2xl shadow-xl">
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-6 text-center">Extract PDF Pages</h1>

            <div className="text-gray-600 dark:text-gray-300 mb-8 text-center">
                Extract specific pages from your PDF document.
            </div>

            <form onSubmit={handleExtractPages} className="space-y-6">
                {/* File Upload */}
                <div>
                ...
                <button
                    type="submit"
                    disabled={status.loading || !file || !pages.trim()}
                    className={`w-full flex items-center justify-center py-4 px-6 rounded-xl font-semibold text-white transition-all duration-300 ${
                        status.loading || !file || !pages.trim()
            ...
            <div className="mt-12 p-6 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-2xl border border-blue-100 dark:border-blue-800/30">
                ...
                <ol className="list-decimal list-inside space-y-2 text-blue-700 dark:text-blue-300">
                    ...
                    <li>Enter the page numbers you want to extract in the "Pages to Extract" field (e.g., "1,3-5").</li>
                    <li>Click the "Extract Pages" button.</li>
                    <li>Once the process is complete, a download link will appear. Click it to download your extracted PDF file.</li>
                </ol>
            </div>
        </div>
    );
